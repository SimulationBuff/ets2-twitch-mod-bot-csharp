...
This repository contains a complete rewrite of the ETS2 Twitch Mod Bot in C# targeting .NET 8.
It includes:

- Cross-platform core library (`ETS2TwitchModBot.Core`) that implements the parsing, decryption,
  caching, DLC detection, cooldowns and other logic ported from the original Python project.
- A Windows WPF desktop application (`ETS2TwitchModBot.App`) that provides a simple UI to start/stop
  the bot, view logs, and display recently-detected mods/DLC.
- Unit tests using xUnit in the `tests/` project.
- A GitHub Actions workflow configured to build, test and publish Windows single-file artifacts.

This README describes how to build, test, run and publish the solution.
...
Top-level layout under `ets2-twitch-mod-bot-csharp/` (created in this workspace):
...
(If a directory name doesn't exactly match the above, check the solution file after scaffold.)
...
3. Commands from chat are processed and route to core logic (cooldown checks, parsing, formatting).
...
You can find the workflow under `.github/workflows/dotnet-windows.yml` (or similarly named file). The workflow requires repository secrets for any publishing/upload tokens if you integrate additional services.
...
- Finish scaffolding the solution files and projects with code stubs for the core components and WPF UI.
- Create the GitHub repository `ets2-twitch-mod-bot-csharp` under the `SimulationBuff` account and push the initial commit, including the GitHub Actions workflow.
- Implement the full port (core + UI + tests) in an initial pass (Option A).

Please confirm and I will proceed to scaffold and implement the first pass of the .NET solution and push it to the new repository.
